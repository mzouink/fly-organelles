...
def make_data_pipeline(
    ...
    for high in (16,32):
        for low in (64,128):
            t_high_res = gp.Coordinate((high, high, high))
            t_low_res = gp.Coordinate((low, low, low))
...
# %%
batch[low_label_key].data.shape
#%%
array = batch[low_label_key].data
from scipy.ndimage import zoom
