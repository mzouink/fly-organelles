...
def _compute_affs_single(
    ...
    lower = tuple(slice(max(0, -o), min(shp[i], shp[i] - o)) for i, o in enumerate(offset))
    upper = tuple(slice(max(0, o), min(shp[i], shp[i] + o)) for i, o in enumerate(offset))
...
def get_affs(
    ...
    *,
    ...
    aff_list = [_compute_affs_single(np.asarray(arr), offset, dist_fn, pad) for offset in neighborhood]
