...
def create_lv_stacked(checkpoints_path,crop, volume_type="segmentation", array_name="raw", offset=None, voxel_size=None):
    dask_arrs = []
    for checkpoint in checkpoints_path:
        z_arr = fst.access(os.path.join(checkpoint, crop, array_name),mode="a")
        if offset is not None:
            z_arr.attrs["offset"] = offset
        if voxel_size is not None:
            z_arr.attrs["voxel_size"] = voxel_size
...
def set_layers(state, checkpoints_path, crop, organelle, raw_path, gt_path,resolution):
    ...
    state.layers["raw"] = neuroglancer.ImageLayer(
        source=mounted_raw_path
    )
    state.layers["labels"] = neuroglancer.SegmentationLayer(
        source=mounted_gt_path
    )
    state.layers["predictions"] = neuroglancer.ImageLayer(
        source=create_lv_stacked(checkpoints_path, crop, volume_type="image", array_name=organelle, offset=gt_offset, voxel_size=resolution)
    )
...
def main(run_path):
    ...
    checkpoints_path = [os.path.join(validation_path, d) for d in os.listdir(validation_path) if d.startswith("model_checkpoint_") and d.endswith(".zarr")]
    crops = [d for d in os.listdir(checkpoints_path[0]) if os.path.isdir(os.path.join(checkpoints_path[0], d)) ]
    organelles = [d for d in os.listdir(os.path.join(checkpoints_path[0], crops[0])) if os.path.isdir(os.path.join(checkpoints_path[0], crops[0], d)) ]
...
main("/groups/cellmap/cellmap/zouinkhim/exp_cerebellum/runs/setup_11")
